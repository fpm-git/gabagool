...
function generateStandaloneModelDefinition(model) {
    ...
    const instanceClass = `export declare abstract class $${model.name}Instance {${classProps}}`;
    ...
    (model.functions || []).forEach(func => {
        if (['beforeCreate', 'afterCreate', 'beforeUpdate', 'afterUpdate', 'beforeDestroy', 'afterDestroy'].indexOf(func.name) === -1) {
