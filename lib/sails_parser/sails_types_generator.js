...
function resolveFunctionTypes(modelOrService, modelsCollection) {
    ...
    function getTypedParameters(func) {
        function getTypesForParam(name) {
            ...
            // If we've a rest parameter, it's JSDoc definition won't have the whole ... prefix, so remove it for the search
            if (name.startsWith('...')) {
                name = name.substr(3);
            }
    ...
    (functions || []).forEach(func => {
        ...
        (func.paramsTyped || []).forEach(param => {
            ...
            if (param.name.startsWith('...')) {
                param.types = param.types.map(t => (t.startsWith('...') ? t.substr(3) : t) + '[]');
            }
